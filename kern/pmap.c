...
boot_alloc(uint32_t n)
{
	...
	char* expected_next = nextfree + ROUNDUP(n , PGSIZE);
	if((size_t)expected_next - KERNBASE > npages * PGSIZE)
	{
		panic("out of memory!!!\n");
	}
	else
...
mem_init(void)
{
	...
	// Your code goes here:
		// even I dont want to write a second annotation, but this trick is amazing !
	...
	boot_map_region(kern_pgdir , IOMEMBASE , -PGSIZE  - IOMEMBASE , IOMEMBASE , PTE_W);
...
mem_init_mp(void)
{
	...
	//boot_map_region(kern_pgdir , IOMEMBASE , -PGSIZE , IOMEMBASE , PTE_W);
...
page_init(void)
{
	...
	//cprintf("page_init\n");
	uint32_t i;
	uint32_t io_start = IOPHYSMEM / PGSIZE;
	uint32_t io_end = EXTPHYSMEM / PGSIZE;
	uint32_t allocated_start = 0x100000 / PGSIZE;
						// where the kernel is actually located;
	uint32_t allocated_end = PADDR(boot_alloc(0)) / PGSIZE; 
						// where we have already allocated up to;
	pages[0].pp_ref = 1;
	...
	for( ; i < allocated_start; i++)
	{
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	}
	for( ; i < allocated_end ; i++)
	{
		pages[i].pp_ref = 1;
	}
	for(; i < npages ; i++)
	...
	/*struct Page* abp = pa2page(MPENTRY_PADDR);
	abp->pp_ref = 1;
	abp++;
	abp->pp_link = abp - 2;*/
	/*
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if((pgdir[pdx] & PTE_P) == 0)
	{
		if(create)
		{
			...
			new_page->pp_ref ++;
			pgdir[pdx] = PTE_ADDR(page2pa(new_page)) | PTE_P | PTE_W | PTE_U;
			thePTE = page2kva(new_page);
		}
		else
		{
			return NULL;
	...
	{
		thePTE = page2kva(pa2page(PTE_ADDR(pgdir[pdx])));
	}
	return thePTE + ptx;
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	{
		thePTE = pgdir_walk(pgdir , (void*)(va + i) , 1);
		if(thePTE == NULL)
		{
			panic("out of memory!!!\n");
		}
		*thePTE = PTE_ADDR(pa + i) | perm | PTE_P;
	}
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	...
	pte_t* thePTE = pgdir_walk(pgdir , va , 1);
	if(thePTE == NULL)
	{
		return -E_NO_MEM;
	}
	else
	{
		...
		*thePTE = PTE_ADDR(page2pa(pp)) | PTE_P | perm;
		return 0;
	}
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	pte_t* pte_store;
	struct Page* thePage;
	thePage = page_lookup(pgdir , va , &pte_store);
	if(thePage == NULL)
	{
		return;
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	//cprintf("begin to check: %x to %x \n" , (uint32_t)va , (uint32_t) va + len);

	char* va_left = ROUNDDOWN((char*)va , PGSIZE);
	char* va_right = ROUNDUP((char*)va + len , PGSIZE);
	pte_t* pte;
	for(; va_left < va_right ; va_left += PGSIZE)
	{
		if((va_left > (char*)ULIM) || ((pte = pgdir_walk(env->env_pgdir , va_left , 0)) == NULL) || ((*pte & perm) != perm))
		{

			if(va_left == ROUNDDOWN((char*)va , PGSIZE))
			{
				// even a trap here??????
				// li hai le word ge 
			...
			{
				user_mem_check_addr = (uintptr_t) va_left;
			}
			return -E_FAULT;
...
/*
	...
	*/
	/*
	int i;
	unsigned int begin = (unsigned int) va / PGSIZE * PGSIZE + PGSIZE;
